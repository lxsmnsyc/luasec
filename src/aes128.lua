...
local ltable = {
[0]=0x00, 0xff, 0xc8, 0x08, 0x91, 0x10, 0xd0, 0x36, 0x5a, 0x3e, 0xd8, 0x43, 0x99, 0x77, 0xfe, 0x18, 
...
local atable = {
[0]=0x01, 0xe5, 0x4c, 0xb5, 0xfb, 0x9f, 0xfc, 0x12, 0x03, 0x34, 0xd4, 0xc4, 0x16, 0xba, 0x1f, 0x36, 
...
local function InvSubBytes(a)
    for i = 0, 15 do
        a[i] = rsbox[a[i]]
    end
end 

local function ShiftRows(a)
    --[[
        0 4 8 C | 0 4 8 C
        1 5 9 D | 5 9 D 1
        2 6 A E | A E 2 6
    ...
    a[0x1], a[0x5], a[0x9], a[0xD] = a[0x5], a[0x9], a[0xD], a[0x1]
    a[0x2], a[0x6], a[0xA], a[0xE] = a[0xA], a[0xE], a[0x2], a[0x6]
    a[0x3], a[0x7], a[0xB], a[0xF] = a[0xF], a[0x3], a[0x7], a[0xB]
end 

local function InvShiftRows(a)
    --[[
        0 4 8 C | 0 4 8 C
        1 5 9 D | D 1 5 9
        2 6 A E | A E 2 6
    ...
    a[0x1], a[0x5], a[0x9], a[0xD] = a[0xD], a[0x1], a[0x5], a[0x9]
    a[0x2], a[0x6], a[0xA], a[0xE] = a[0xA], a[0xE], a[0x2], a[0x6]
    a[0x3], a[0x7], a[0xB], a[0xF] = a[0x7], a[0xB], a[0xF], a[0x3]
end
...
local function AddRoundKey(data, cipher)
    for i = 0, 15 do
        data[i] = gadd(data[i], cipher[i])
    end
end

local function KeySchedule(cipher, round, cipherstore)
    cipherstore[0x0] = gadd(cipher[0x0], sbox[cipher[0x7]], rcon[round])
    cipherstore[0x4] = gadd(cipher[0x4], sbox[cipher[0xB]])
    cipherstore[0x8] = gadd(cipher[0x8], sbox[cipher[0xF]])
    cipherstore[0xC] = gadd(cipher[0xC], sbox[cipher[0x3]])

    cipherstore[0x1] = gadd(cipher[0x1], sbox[cipher[0x0]])
    cipherstore[0x5] = gadd(cipher[0x5], sbox[cipher[0x4]])
    cipherstore[0x9] = gadd(cipher[0x9], sbox[cipher[0x8]])
    cipherstore[0xD] = gadd(cipher[0xD], sbox[cipher[0xC]])

    cipherstore[0x2] = gadd(cipher[0x2], sbox[cipher[0x1]])
    cipherstore[0x6] = gadd(cipher[0x6], sbox[cipher[0x5]])
    cipherstore[0xA] = gadd(cipher[0xA], sbox[cipher[0x9]])
    cipherstore[0xE] = gadd(cipher[0xE], sbox[cipher[0xD]])
    
    cipherstore[0x3] = gadd(cipher[0x3], sbox[cipher[0x2]])
    cipherstore[0x7] = gadd(cipher[0x7], sbox[cipher[0x6]])
    cipherstore[0xB] = gadd(cipher[0xB], sbox[cipher[0xA]])
    cipherstore[0xF] = gadd(cipher[0xF], sbox[cipher[0xE]])
    return cipherstore
end 
...
local function new(cipher)
    ...
    ciphers[0] = cipher
    for i = 1, 10 do
        ciphers[i] = KeySchedule(ciphers[i - 1], i, {})
    end

    local aes = {ciphers = ciphers}
    setmetatable(aes, m)
    return aes 
end 
